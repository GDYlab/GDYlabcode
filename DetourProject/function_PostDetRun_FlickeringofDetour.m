function [revdecode,mdetotpbdir] = function_PostDetRun_FlickeringofDetour(ses,mazel,M1,M2,PlFields,PlFmesh,ClF,T2T4Laps,tbin,overlap)
% function_PostDetRun_FlickeringofDetour
...
% outputs:  revdecode is a structure with dimension (direction,track,session)
%                     revdecode(idir,tnum,is).seg(seslap,iseg) containing
...
%                                    2. 150cm detour segment 
%                                    3. 50cm revesal segment 
%                                    4. 150cm control segment 
%                                    5. 50cm end stationary segment 
%                     revdecode(idir,tnum,is).seg(seslap,iseg).pdf is the
%                     decoding probability with D1 being spatial bins, and
%                     D2 being time bins
%                     revdecode(idir,tnum,is).seg(seslap,iseg).tbin specify
%                     the time bins (n*2 matrix as start and end of time bins)
%                     revdecode(idir,tnum,is).seg(seslap,iseg).tbin specify
%                     the spatial mesh 
%                     revdecode(idir,tnum,is).seg(seslap,iseg).nact is the
%                     number of active pyr cells in this bin
%                     revdecode(idir,tnum,is).seg(seslap,iseg).vel is the
%                     averaged run velocity of this time bin
%         mdetotpbdir  is an array of averaged decoding probabilities on
%                     detour and control segments,
%                     D1 sessions, only detour session has value, 
%                     D2 directions, D3 detour and control pb
%
...
%% decoding post-detour run laps, we will add current reversal, two stationary linear segments
% detour segment, and control segment to the decoder
revdecode = struct;
...
for idir = 1:2
    for it = 1:length(dettra)
        ...
        % activity session loop, post detour session
        is = detses+1;
        ...
        for il = 1:size(dirt,1)
            ...
            % make sure the run should cover 50% of the track
            % length
            ...
            % the start and the end of track; 10 cm reversal segment
            % 150 cm detour segment, 150 cm control segment as the opposite track
            addses = [is,is-1,is,is-1,is];
            ...
            addlpos{4} = [ses(is-1).tralim(othert,1), ses(is-1).tralim(othert,2)];
            ...
            for iseg = 1:size(lposlimit,1)
                ...
                revdecode(idir,tnum,is).seg(seslap,iseg).pdf = dprob;
                revdecode(idir,tnum,is).seg(seslap,iseg).tbin = sframes.tbin;
                revdecode(idir,tnum,is).seg(seslap,iseg).spamesh = spamesh;
                revdecode(idir,tnum,is).seg(seslap,iseg).nact = sframes.nact;
                % get velocity
                ctbin = nanmean(sframes.tbin,2);
                revdecode(idir,tnum,is).seg(seslap,iseg).vel = interp1(mazelt,abs(mazelvel),ctbin);
            end
            seslap = seslap + 1;
        end
    end
...
mdetotpbdir = nan(4,2,2);
% D1 sessions, only post-detour session has value, D2 directions, D3 detour and control pb

for is = detsesnum + 1
    tnum = setdiff([1,2,3,4],ses(is-1).tra_p);
    % find detour track in detour sessions
    for idir = 1:2
        dcdtmp = revdecode(idir,tnum,is).seg;
        nlap = size(dcdtmp,1);
        pdata = [];
        for ilap = 1:nlap
            goodbin = berow(dcdtmp(ilap,2).nact) >= actcri & berow(dcdtmp(ilap,2).vel) >= vcri;
            detpb = dcdtmp(ilap,2).pdf(:,goodbin);
        ...
        mdetotpbdir(is,idir,1) = nanmean(pdata(:,1));
        mdetotpbdir(is,idir,2) = nanmean(pdata(:,2));
    end
end
