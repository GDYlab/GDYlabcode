function [out, t, Pairs] = CCG_zyc(T, G, BinSize, HalfBins, SampleRate, GSubset, Normalization)
...
% Construct the path to the CCGEngine executable
CCGEnginePath = fullfile(currentFolder, 'CCGEngine');
% Format the command
Command = sprintf('!%s %d %s %s %f %d %s %s', ...
    CCGEnginePath, nSpikes, ResFile, CluFile, BinSize, HalfBins, OutFile, PairFile);
% Run the command
eval(Command);
