function varargout=sigstar(groups,stats,nosort,starsize,starty)
   ...
   if nargin<5
    ...
    y=ylim;
    yd=myRange(y)*0.13; %separate sig bars vertically by 5% 
    ...
    for ii=1:length(groups)
        ...
        H(ii,:)=makeSignificanceBar(xlocs(ii,:),thisY,stats(ii),starsize);
    end
    %-----------------------------------------------------
...
function H=makeSignificanceBar(x,y,p,starsize)
    ...
    H(1)=plot(x(:),y,'-','color',[1 1 1],'LineWidth',1.5,'Tag','sigstar_bar');
    end
    ...
    H(2)=text(mean(x(:)),starY,stars,...
        ...
        'Tag','sigstar_stars','fontsize',starsize);
