function MyBoxPlot_v3(d,ax,varargin)
...
for i = 1:length(d)
    ...
    IQR = abs(diff(boxedge));
    wisrange = [boxedge(1)-1.5*IQR,boxedge(2)+1.5*IQR];
    pdfrange = [ptemp(max(1,round(dlen*0.001))),ptemp(round(dlen*0.999))];
    allmin = min([allmin,pdfrange(1)]);
    allmax = max([allmax,pdfrange(2)]);
    mediap = median(ptemp);
    
    datainidx = idxinrange(ptemp,wisrange);
    wisrange(1) = min(ptemp(datainidx));
    wisrange(2) = max(ptemp(datainidx));
