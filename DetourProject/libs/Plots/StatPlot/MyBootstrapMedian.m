function MyBootstrapMedian(d,ax,baseline,range,its,datalabel,morperc)
%MyBootstrapMedian compare group of observations specified in d and plot in axis ax
...
%
%         morperc: plot mean+-std (0) or median+-25%75% (1), median+-25%75% as default
%
% example:  
% %          a = rand(100,1);b = rand(20,20); c = rand(1,15);
% %          [~,pab] = ttest2(a,b(:)); [~,pac] = ttest2(a,c); [~,pbc] = ttest2(b(:),c);
% %          MyBootstrapMedian({a,b,c},gca,1)
...
for i = 1:length(d)
    ...
    % plot mean and std
    ptemp = berow(ptemp);
    plot(ax,i,nanmean(ptemp),'s','color',colors{colori},'markersize',4,'linewidth',4);
    plot(ax,[i i],[nanmean(ptemp)-nanstd(ptemp),nanmean(ptemp)+nanstd(ptemp)],'-','color',colors{colori},'linewidth',1.5);
    else
    % plot median and 25% 75%
    ptemp = berow(ptemp);
    [ptemp,~] = sort(ptemp,'ascend');
    plot(ax,i,nanmedian(ptemp),'s','color',colors{colori},'markersize',4,'linewidth',4);
...
% %     fill(xx, yy, 1-1*(1-colors{i}),'LineStyle','none','FaceAlpha',0.6);
    ...
    hdata = plot(ax,plotx,ptemp,'o','color',colors{colori},'markersize',2,'linewidth',2);
    if ~isempty(datalabel) && ~isempty(labtemp)
        customDataCursor(hdata,labtemp);
    end
    ...
    mtemp = bootstrp(its,@median,ptemp);
