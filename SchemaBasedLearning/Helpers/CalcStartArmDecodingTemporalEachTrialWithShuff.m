...
%          dprob(isnan(dprob))=0;
 bins=[1:25:151];MeanProb=[];
 for dd1=1:300
 shuffids=circshift([1:150], round(rand(1).*150));    
 dprob1=dprob(shuffids,:);
 for ss=1:6
meanPP=((dprob1(bins(ss):bins(ss+1)-1,sel)));
meanPP=movmean(meanPP,[1 1],2,'omitnan');
MeanProb(dd1,rr,ss,:,:)=meanPP;
 end
 end
...
GoalProbs=[];sel=80:220;
MeanProbs2=[];Errors2=[];pp=1;MeanProbsShuff2=[];
 for ii=1:25
     tic;
GenPath=strcat(paths,files{ii});
cd(GenPath{1})
...
MeanProbsShuff2{ii,gg,1}=MeanProbTargShuff;
MeanProbsShuff2{ii,gg,2}=MeanProbNextShuff;
MeanProbsShuff2{ii,gg,3}=MeanProblastShuff;
Errors2{ii,gg}=Err;
     toc
 end
display(strcat('DoneForSess',num2str(ii))) 
...
GenPath=strcat(paths1{2},'Results/PlotFigures/Database');
cd(GenPath)
save('StartArmDecodingTemporal50Shuff','MeanProbs1','MeanProbsShuff1',...
    'Errors1','MeanProbs2','MeanProbsShuff2','Errors2','-v7.3') 
